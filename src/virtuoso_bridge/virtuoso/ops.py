...
def bind_or_open_cell_view(
    ...
    *,
    ...
    """Build a SKILL expression that reuses any open window for lib/cell/view.

    Iterates hiGetWindowList() and matches on the lib+cell+view triple.
    If a matching window is found it is raised and its cellview is returned;
    otherwise dbOpenCellViewByType opens a fresh one.  The result is a bare
    SKILL expression (no ``cv =`` assignment) so callers can wrap it freely.
    """
    el = escape_skill_string
    resolved_view_type = view_type or default_view_type_for(view)
    return (
        f'let((rbFound rbWin) '
        f'foreach(rbWin hiGetWindowList() '
        f'  when(!rbFound && rbWin~>cellView '
        f'    && rbWin~>cellView~>libName  == "{el(lib)}" '
        f'    && rbWin~>cellView~>cellName == "{el(cell)}" '
        f'    && rbWin~>cellView~>viewName == "{el(view)}" '
        f'    rbFound = rbWin)) '
        f'if(rbFound '
        f'  then progn(hiSetCurrentWindow(rbFound) rbFound~>cellView) '
        f'  else dbOpenCellViewByType("{el(lib)}" "{el(cell)}" '
        f'    "{el(view)}" "{el(resolved_view_type)}" "{el(mode)}")))'
    )
